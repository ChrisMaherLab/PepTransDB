...
Usage: python sorf_to_genome.py <ORF file> <exon file> <geneid file>

Args: 
<ORF file> = reformatted FASTA file of ORFs from getORF
...
columns (0 based numbering) 0:chromosome, 3:transcript ID, 4:strand, 8:exons (format start;stop,start;stop etc. )

<geneid file> = file with gene ID in last column and transcript name in column 7 (0 based numbering) 
"""
...
name_list = []
#dictionary of transctipt IDs (key) and chromosomes (value)
chrom_dict = {}
#dictionary of transcript IDs (key) and their exons in a list of lists (value)
exon_dict = {}
#dictionary of transcript IDs (key) and strands (value)
strand_dict = {}
...
geneid_dict = load_obj('geneid_dict_for_orfs')

#if have saved comment out this section ##################################################
#otherwise generate dictionary transcript ID (value) geneid (key)
geneid_dict = {}
#get info from <geneid file>
with open(sys.argv[3],'r') as genes:
	for line in genes:
		linesplit = line.split()
		if linesplit[7] in name_list:
			if linesplit[7] not in geneid_dict.keys():
				geneid_dict[linesplit[7]] = [linesplit[-1]]
			elif linesplit[7] in geneid_dict.keys():
				geneid_dict[linesplit[7]] += [linesplit[-1]]
			else:
				continue
		else:
			continue
#for saving dictionary 
save_obj(geneid_dict, 'geneid_dict_for_orfs')
########################################################################################

#remove duplicates (becasue starting file was by exon, not transcript)
for key,value in geneid_dict.items():
	geneid_dict[key] = set(value)
